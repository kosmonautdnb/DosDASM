...
*/
// Stefan Mader did modify the source code to compile with OpenWatcom2.0 on 03 November 2025 (needed by GPL)
    ...
    if (mlflag) // a previous multi-line comment is in progress...
    {
        if ( mlend )
        {
            ...
            *mlend = 0;
            ...
            sprintf(tempbuf,"%s;*/%s",mlend+2,buf); // put the comment at the end of the line
        ...
        if (mlstart && ((!semistart) || (mlstart < semistart)))
        {
            if (mlend && (mlstart < mlend) && ((!semistart)||(mlend < semistart))) // single line /* */
            {
                ...
                *(mlend+1)=0;
                ...
                sprintf(tempbuf,"%s%s;/%s",buf,mlend+2,mlstart+1); // move the first comment to the end of the line
                strcpy(buf,tempbuf);
                return(cleanup(buf,bDisable)); // repeat for any single-line comments that may follow
            }
        ...
        if (buf[i] == ',')  // we have label arguments
        {
            ...
            else // or else it's a symbol to be evaluated, and added to the label
            {
                ...
                if(symarg)
                {
                    ...
                    {
                        sprintf(tempval,"%d",(unsigned)symarg->value);
