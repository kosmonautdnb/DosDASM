...
{
    return (ERRORFORMAT_MIN2 <= format && format <= ERRORFORMAT_MAX2);
}
 ...
 */
static size_t
sane_vsnprintf(/*@out@*/ char *str, size_t size, const char *fmt, va_list ap)
{
    int res;
    assert(str != NULL);
    assert(size > 0);
...
static size_t
sane_snprintf(/*@out@*/ char *str, size_t size, const char *fmt, ...)
